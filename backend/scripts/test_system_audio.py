#!/usr/bin/env python3
"""
Test system audio capture - captures system audio and saves to WAV file.
Play a video/music during the capture, then play back the WAV to verify.
"""
import asyncio
import sys
import wave
...
OUTPUT_FILE = "test_system_audio.wav"
SAMPLE_RATE = 16000


async def test_system_audio():
    print("=" * 60)
    print("SYSTEM AUDIO CAPTURE TEST")
    print("=" * 60)
    print(f"\nThis will capture {CAPTURE_DURATION} seconds of SYSTEM audio.")
    print("Please play a video or music during the capture.\n")

    input("Press ENTER to start capturing system audio...")
    ...
    try:
        async for chunk in sck.capture_audio(capture_system=True, capture_mic=False):
    ...
    if all_audio:
        ...
        print("\nIf the playback sounds correct, the audio capture is working!")
