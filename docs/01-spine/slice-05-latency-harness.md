...
- [ ] **BLOCKER on live check 5:** fix the latency anchor — volatile `range.end` is a forward-looking window boundary, not consumed audio (98.2% of long-fixture samples negative; p95 itself negative). Investigate per-run `.audioTimeRange` attributes vs a fed-audio clamp; add excluded-impossible-sample reporting to HarnessReport
