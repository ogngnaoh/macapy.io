...
**Status:** shipped 2026-07-17 (close-out record in Integration notes — criteria 2/4 closed by owner decision, see note)
...
- 2026-07-17 (close-out — milestone marked shipped): **Criterion 5 cold start PASS with evidence**: Debug build, LaunchServices launch→checkin **0.417s cold** / 0.025s warm (observed `open`→checkin wall 0.053s), ~5× inside the 2s budget; pause/ephemeral halves proven in slice 4. **Criteria 2 and 4 were closed by owner decision without machine evidence** ("force the close out", 2026-07-17): a background `lsof` monitor was clean 67/67 polls over its ~6-min observed window, but the DB shows **no persisted meeting from that window** — the real-meeting dogfood (dual-source live → persisted → reopened from History) and zero-network-during-a-full-meeting were not evidenced in this session; the author attested them done. They will be re-proven incidentally by M2's exit criteria (real-meeting artifact + diarization checks). **Defect found during close-out, carried to M2**: the test suite writes to the production DB — `AppShellCoordinator`'s default `makePersistentStore` falls through to `productionMeetingStore` (AppShellCoordinator.swift:47,83); proven by meeting count 85 → 87 across one `swift test --filter AppShellTests` run. All 87 rows in the live DB are sub-second test artifacts (us-only segments); cleanup pending owner go-ahead.
