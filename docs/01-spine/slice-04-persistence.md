...
- 2026-07-17 (builder, flush fix, 98b75cf → d6abb55): mechanism — new `TranscriptStore.finishFinalsStreams()` (ends finals continuations without clearing segments/volatile; reportable TranscribeKit API deviation), `MeetingPipeline.stop()` calls it after draining event tasks, `SegmentWriter.run()` drains to natural stream termination and flushes at loop exit, `flushAndStop()` awaits run's completion signal (CheckedContinuation) and rethrows. Invariant now rests on AsyncStream's yielded-before-finish delivery guarantee, not actor job ordering. Striking result: with the yield-slack removed, the pre-fix race was **deterministic, not flaky** — `run()` never got scheduled before `flushAndStop()` read an empty buffer (51 identical failures × 8 runs) — i.e. every real meeting stop was losing tail finals whenever timing matched the test's shape; the original test passed only because of its scheduling slack. New 50-iteration immediate-stop stress test; 56/56 ×4 post-fix.
- 2026-07-17 (verifier, fresh context): checks 1–7 PASS; binding-constraint tests confirmed real (converse loss test demonstrated, not decorative); ephemeral check contrasts one real on-disk temp file before/after; all pre-existing-test adaptations judged mechanical, zero weakening. **Residual (d), upgraded to fix-before-ship by orchestrator:** `flushAndStop()` depends on actor-executor enqueue order (implementation detail, not a happens-before guarantee); a priority-escalated stop task could flush an empty buffer ahead of the run-loop's pending appends → tail finals of a meeting silently unpersisted; the existing final-flush test inserts 5× `Task.yield()` which sidesteps exactly this race. Fix direction: finish the finals stream at stop → run() drains to termination and flushes at exit → stop awaits the writer task; no scheduling slack in tests. Residual (e): HotKey ID-filter sound by inspection; sole edge (GetEventParameter failure → unhandled press) accepted as negligible.
...
- 2026-07-17 (verifier, focused re-check): fix judged "fixed, not papered over." Documented precondition worth keeping visible: `SegmentWriter.flushAndStop()` requires `TranscriptStore.finishFinalsStreams()` (or `reset()`) to have ended the stream first, else it waits forever — `MeetingPipeline.stop()` calls both unconditionally in order, and `finishFinalsStreams()` is idempotent (safe double-call with the next meeting's `reset()`). Same contract class as the attach-before-start BINDING note below.
